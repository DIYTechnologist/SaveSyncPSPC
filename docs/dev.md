...
- **RE3 is the most tractable next title**: owned on both platforms, PC side decodes with the existing `KeyRE3` (now verified against real saves), PS5 side is plaintext. It needs `Decode`/`Build` taught the unencrypted `flags=0x0` layout, after which conversion is the same body-realignment problem RE2 already solved - no new cipher.
