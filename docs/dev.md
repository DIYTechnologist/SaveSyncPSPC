...
- **RE4's PS5 key was recovered** and is now `KeyRE4` in `dsss.go`, confirmed against the real save. The method generalizes and is worth reusing: the encrypted `DSSSDSSS` check block is a known-plaintext oracle, so sweeping every printable-ASCII run in the whole eboot.bin against it identifies the key in seconds - much cheaper than disassembling to find the key-schedule call site, which was tried first and led nowhere. RE4 is *not* convertible yet, though: its Steam build uses "Lime" (ElGamal + AES-OFB keyed off the Steam account ID), so there's no fixed PC-side key and reading a PC save needs a real ElGamal implementation.
...
Adding another title on this engine now means: confirm the PS5 shape (encrypted or not - try parsing at `0xc` first), find its key if encrypted (the `DSSSDSSS`-oracle sweep, `TODO.md` has the method), and find its platform class by diffing real PC/PS5 saves for the two known field hashes - no new Go code beyond a `TitleConfig` entry and a `games/<key>.json`, unless the title turns out to use a different cipher entirely (Lime, Mandarin).
