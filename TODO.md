...
- **RE4 (2023) - wired in, format-confirmed, not yet tested in-game.** `games/re4.json`, engine `reengine`, title `re4`. PS5 side is plain Blowfish (`KeyRE4`, found via a known-plaintext oracle sweep of the real eboot.bin - see `docs/dev-res4.md`). PC (Steam) side uses a completely different cipher, "Lime" - implemented in Go from scratch (`internal/reengine/lime.go`): ElGamal-style modexp over a fixed public prime group (`math/big`), AES-128-OFB (`crypto/aes`), SHA3-256 per-block checksums (`golang.org/x/crypto/sha3`, newly added to `go.sum`). Fully validated against two real Steam saves pulled from a Steam Deck: every block's checksum verifies, full encode/decode round trip is byte-identical, wrong Steam ID is correctly rejected, and the complete PC→PS5→PC conversion round trip produces valid, re-parseable output. Needed a new mechanism since RE4 is the first engine needing a runtime value (Steam ID) no profile can supply ahead of time: new `--steam-id` CLI flag → `bridge.Options.SteamID` → `gameapi.SaveImage.SteamID`, the same non-interface-changing pattern `--ps5-save-name` already uses. RE4's own converter (`ConvertRE4PCToPS5`/`ConvertRE4PS5ToPC` in `internal/reengine/re4.go`) is separate from RE2/RE3's shared `TitleConfig` path, since its two sides are genuinely different cipher families. **Caveat**: `RE4PlatformClass`'s field mapping (class `0x100e60`, same field hashes as RE2/RE3) was found from a single real sample per platform, unlike RE2/RE3's multi-sample confirmation - and the boolean field showed `false` on both sides in that one sample, so it may not be platform-discriminating for RE4 at all. See `docs/dev-res4.md` for the full writeup (the key-discovery method, the exact Lime container/cipher spec, and the CLI wiring).
