...
func (s Server) apiPCSaveSuggestions(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{"candidates": pcpath.Suggest(profile.PCSaveDirs, profile.SteamAppID)})
}
...
async function loadPCSuggestions(){const game=$('game').value; if(!game){$('pcsuggestions').innerHTML='<span class="mut">Select a game first.</span>'; return;} $('pcsuggestions').textContent='Looking...'; try{const data=await api('/api/pc-save-suggestions',{game}); const cands=data.candidates||[]; if(!cands.length){$('pcsuggestions').innerHTML='<span class="mut">No suggestions for this game/OS - type a path manually.</span>'; return;} $('pcsuggestions').innerHTML=cands.map(c=>` + "`" + `<div data-path="${esc(c.path)}" data-isdir="1" style="cursor:pointer;padding:3px 0" title="${esc(c.reason)}">${c.exists?'✓':'?'} ${esc(c.path)} <span class="mut">(${esc(c.reason)})</span></div>` + "`" + `).join('');}catch(e){$('pcsuggestions').innerHTML=` + "`" + `<span class="err">${e.message}</span>` + "`" + `;}}
